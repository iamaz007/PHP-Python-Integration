...
if (isset($_POST['dataType'])) {
    ...
    for ($i = 0; $i < count($temp); $i++) {
        for ($j = 0; $j < count($temp[$i]); $j++) {
            ...
            $link = addslashes($temp[$i][$j]['link']);

            $sql = "INSERT INTO  " . $_POST['dataType'] . " (title, price, image, brand, link)
                VALUES ('" . $title . "', '" . $price . "', '" . $image . "', '" . $brand . "', '" . $link . "')";
                            ...
                            <div class="card mb-4 box-shadow">
                                ...
                                <div class="card-body">
                                    ...
                                    <div class="d-flex justify-content-between align-items-center">
                                        <div class="btn-group">
                                            <a href="/functions.php?itemDetail=' . $row['id'] . '&type=' . $_POST['dataReadType'] . '" class="btn btn-sm btn-outline-secondary">View</a>
...
if (isset($_GET['itemDetail'])) {
    // echo $_GET['itemDetail'];
    $sql = "SELECT * FROM " . $_GET['type'] . " WHERE id = '" . $_GET['itemDetail'] . "'";
    $result = mysqli_query($con, $sql);
    $row = mysqli_fetch_array($result);
    // print_r($row);
    $url = "http://127.0.0.1:5000/getDetails?link=" . urlencode($row['link'])."&brand=".$row['brand'];
    $data = file_get_contents($url);

    $res = json_decode(mb_convert_encoding($data, 'UTF-8', 'UTF-8'), true);

    // display code here or save record in databsae
    print_r($res);
}
